...
describe RequestLogAnalyzer::Aggregator::Database do
  ...
  describe '#prepare' do
    ...
    before(:each) do
      @database_inserter = RequestLogAnalyzer::Aggregator::Database.new(@log_parser)
  ...
  describe '#initialize_orm_module!' do
    ...
    before(:all) { @database_inserter = RequestLogAnalyzer::Aggregator::Database.new(@log_parser) }
  ...
  describe '#create_database_schema!' do
    ...
    it "should create a table for every line type" do
      @database_inserter.should_receive(:create_database_table).with(an_instance_of(RequestLogAnalyzer::LineDefinition)).exactly(testing_format.line_definitions.length).times
      @database_inserter.send :create_database_schema!
    end

    it "should create a ORM for every line type" do
      @database_inserter.should_receive(:create_activerecord_class).with(an_instance_of(RequestLogAnalyzer::LineDefinition)).exactly(testing_format.line_definitions.length).times
      @database_inserter.send :create_database_schema!
    end
  end  

  describe '.parse_database_connection_string' do

    it "should parse a name-value based string" do
      string = 'adapter=sqlite3;database=filename.db'
      RequestLogAnalyzer::Aggregator::Database.parse_database_connection_string(string).should == {:adapter => 'sqlite3', :database => 'filename.db'}
    end
    ...
    it "should parse an URI-based string for MySQL" do
      ...
      RequestLogAnalyzer::Aggregator::Database.parse_database_connection_string(string).should == 
  ...
  test_databases.each do |name, connection|
    ...
    context "using a #{name} database" do
      ...
      before(:each) do
        @database_inserter = RequestLogAnalyzer::Aggregator::Database.new(@log_parser, :database => connection)
        @database_inserter.prepare
      ...
      after(:each) do
        @database_inserter.send :drop_database_schema!
      end


      it "should insert a record in the request table" do
        testing_format.class::Database::Request.count.should == 0
        @database_inserter.aggregate(@incomplete_request)
        testing_format.class::Database::Request.count.should == 1
      end

      it "should insert records in all relevant line tables" do
        @database_inserter.aggregate(@completed_request)
        request = testing_format.class::Database::Request.first
      ...
      it "should log a warning in the warnings table" do
        testing_format.class::Database::Warning.should_receive(:create!).with(hash_including(:warning_type => 'test_warning'))
